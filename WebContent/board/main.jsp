...
.dot > li {border-bottom: 1px dotted; margin: 15px 10px 15px 0px;"}
...
<table style="width: 1200px; height: 1000px;" cellspacing="0" align="center">
...
<table border="0" style="height: 1000px; width: 1200px;">
...
<td style="width: 250px;">
	...
	<table border="0" style="margin: 10px;">
		<tr style="height: 30px;">
			<td style="width:240px; ;background-color: #242424; border-top-left-radius:10px;border-top-right-radius:10px;">
			<font style="font-size: 13px;"color="#ffffff"><span class="glyphicon glyphicon-info-sign" style="margin: 5px;"></span> <b>Notice</b></font>
			</td>
		</tr>
		<tr style="height: 250px;">
			<td style="width: 240px;background-color: #EAEAEA;">
				<ul class="dot" style="width: 200px;">
					<c:forEach var="dto" items="${notice}" end="6">
                     
                     <li><a href="javascript:location.href='<%=cp%>/sboard/view.do?board=notice&num=${dto.num}';">${dto.subject}</a></li>
...
<td style="width: 450px;" rowspan="2">
	<table border="0" style="margin: 10px;">
		<tr>
			<td style="height: 30px; width: 150px; background-color: #242424; border-top-left-radius:10px;border-top-right-radius:10px;"> 
				<font style="font-size: 13px;"color="#ffffff"><span class="glyphicon glyphicon-thumbs-up" style="margin: 5px 5px 5px 10px;"></span> <b>BestTop5</b></font>
				</td>
			<td></td>
		</tr>
		<tr style="height: 550px;">
			<td style="width: 450px;background-color: #EAEAEA;" colspan="2" >

<table border="0" style="margin-left: 20px; ">
           <c:forEach var="dto" items="${top }" end="5">
               <tr style="height: 100px;">
               <td>
               <c:if test="${dto.savePicture!=null}">
               <img alt="<%= cp %>/images/punch.png" src="${imagePath}/${dto.board}/${dto.num}/${dto.savePicture}" style="max-width: 100px; max-height: 120px;">
                </c:if>
                <c:if test="${dto.savePicture==null}">
              <img alt="" src="<%= cp %>/images/punch.png" style="width: 80px; height: 80px;">
              </c:if>
               </td><td style="width: 300px;"><a href="javascript:location.href='<%=cp%>/sboard/view.do?board=${dto.board}&num=${dto.num}';"><font style="margin-left: 20px;">${dto.subject}</font></a></td>
               </tr>
               </c:forEach>
            </table>
...
<td style="width: 450px;" rowspan="2">
	<table border="0" style="margin: 10px;">
		<tr>
			<td style="height: 30px; width: 150px; background-color: #242424; border-top-left-radius:10px;border-top-right-radius:10px;">
			<font style="font-size: 13px;"color="#ffffff"><span class="glyphicon glyphicon-thumbs-up" style="margin: 5px 5px 5px 10px;"></span> <b>PopularityForum</b></font></td>
			<td></td>
		</tr>
		<tr style="height: 550px;">
			<td style="width: 450px;background-color: #EAEAEA;" colspan="2">
			<table border="0">
				...
				<c:forEach var="dto" items="${freeForum }" end="22">
                     
                     <li><a href="javascript:location.href='<%=cp%>/sboard/view.do?board=freeForum&num=${dto.num}';">${dto.subject}</a></li>
...
<td>
	<table border="0" style="margin: 10px;">
		<tr style="height: 30px; width: 240px;">
			<td style="width:240px; ;background-color: #242424; border-top-left-radius:10px;border-top-right-radius:10px;">
			<font style="font-size: 13px;"color="#ffffff"><span class="glyphicon glyphicon-certificate" style="margin: 5px 5px 5px 10px;"></span> <b>LastPost</b></font></td>
		</tr>
		<tr style="height: 250px;">
			<td style="width: 240px; background-color: #EAEAEA;">
			<!-- 최근본게시물출력 -->
...
if(ck!=null){
	if(ck.length<=6){
		for(int i = ck.length-1;i>=0;i--){
			if(URLDecoder.decode(ck[i].getValue(),"UTF-8").indexOf("board")!=-1){
		
		%>
	<li style="margin: 15px 10px 15px 0px;"><a href="javascript:location.href='<%=cp%>/sboard/view.do<%=URLDecoder.decode(ck[i].getValue(),"UTF-8") %>';"><%=URLDecoder.decode(ck[i].getName(),"UTF-8")%></a>
		<BR/></li>	
<%		
			}

		}
		for(int j= 0 ;j<=6-ck.length;j++){
			%>
			<br>
			<%
		}
			 ...
			 %>	
<%	for(int i = ck.length-1;i>=ck.length-6;i--){
		if(URLDecoder.decode(ck[i].getValue(),"UTF-8").indexOf("board")!=-1){
...
<div id="carousel-example-generic" class="carousel slide" data-ride="carousel" align="center">
  ...
  <div class="carousel-inner" role="listbox">
    <div class="item active">
      <img src="../images/검사.jpg" onclick="javascript:location.href='<%=cp %>/sboard/view.do?board=korMovie&category=&pageNum=&searchKey=subject&searchValue=&num=1'"  style="width: 180px;height: 260px; float: left; margin-left: 62px; margin-right: 10px;">
       <img src="../images/내부자들.jpg" onclick="javascript:location.href='<%=cp %>/sboard/view.do?board=korMovie&category=&pageNum=&searchKey=subject&searchValue=&num=2'" style="width: 180px;height: 260px; float: left;margin-left: 10px; margin-right: 10px;">
      <img src="../images/대호.jpg" onclick="javascript:location.href='<%=cp %>/sboard/view.do?board=korMovie&category=&pageNum=&searchKey=subject&searchValue=&num=3'" style="width: 180px;height: 260px; float: left;margin-left: 10px; margin-right: 60px;">
      
    </div>
    <div class="item">
      <img src="../images/몬스터.jpg" onclick="javascript:location.href='<%=cp %>/sboard/view.do?board=newMovie&category=&pageNum=&searchKey=subject&searchValue=&num=1';" style="width: 180px;height: 260px; float: left; margin-left: 62px; margin-right: 10px;">
       <img src="../images/뽀로로.jpg" onclick="javascript:location.href='<%=cp %>/sboard/view.do?board=newMovie&category=&pageNum=&searchKey=subject&searchValue=&num=2';" style="width: 180px;height: 260px; float: left;margin-left: 10px; margin-right: 10px;">
      <img src="../images/스누피.jpg" onclick="javascript:location.href='<%=cp %>/sboard/view.do?board=newMovie&category=&pageNum=&searchKey=subject&searchValue=&num=3';" style="width: 180px;height: 260px; float: left;margin-left: 10px; margin-right: 62px;">

    </div>
    <div class="item">
       <img src="../images/스타워즈.jpg" onclick="javascript:location.href='<%=cp %>/sboard/view.do?board=newMovie&category=&pageNum=&searchKey=subject&searchValue=&num=4';" style="width: 180px;height: 260px; float: left; margin-left: 62px; margin-right: 10px;">
       <img src="../images/시카리오.jpg" onclick="javascript:location.href='<%=cp %>/sboard/view.do?board=newMovie&category=&pageNum=&searchKey=subject&searchValue=&num=5';" style="width: 180px;height: 260px; float: left;margin-left: 10px; margin-right: 10px;">
      <img src="../images/어린왕자.jpg" onclick="javascript:location.href='<%=cp %>/sboard/view.do?board=newMovie&category=&pageNum=&searchKey=subject&searchValue=&num=6';" style="width: 180px;height: 260px; float: left;margin-left: 10px; margin-right: 62px;">
...
<td>
	<table border="0" style="margin: 10px;" >
		<tr>
			<td style="height: 30px; width: 150px; background-color: #242424; border-top-left-radius:10px;border-top-right-radius:10px;">
			<font style="font-size: 13px;"color="#ffffff"><span class="glyphicon glyphicon-question-sign" style="margin: 5px 5px 5px 10px;"></span>
			<b><A href="javascript:location.href='<%=cp %>/sboard/view.do?board=notice&num=4';"> Question</A></b></font></td>
			<td></td>
		</tr>
		<tr style="height: 250px;">
			<td style="width: 450px; background-color: #EAEAEA;" colspan="2">
			...
			※ 포인트 충전이 안됩니다 <BR>
			☞ 입력 하실때 핸드폰 번호를 정확히 입력해 주시길 바랍니다. <BR>
<BR>
			※ 파일 업로드 시 파일 용량은 어떻게 되나요? <BR>
			☞ 업로드시 용량 제한은 10GB 내에서 가능합니다. <BR>
<BR>
			※ 파일 다운로드가 안됩니다 <BR>
			☞ 다운로드시 포인트가 10씩 차감되오니 포인트를 확인해주시길 바랍니다. <BR>
			<BR>
...
<font style="font-size: 13px; font-family: 고딕;color: #cccccc">Copyright (c) 2015 <span style="font-size: 15px;color: #00D8FF; font-weight: bold;">PUNCH TORRENT</span> 
All rights reserved .  <span style="font-size: 15px;color: #00D8FF;">DMCA</span><br/>
 <span style="font-size: 15px;color: #00D8FF;">광고제휴문의</span> <span style="font-size: 13px;color: #00D8FF;"> contact us</span> dung8758@naver.com</font> </td>
