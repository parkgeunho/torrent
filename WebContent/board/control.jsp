...
<script type="text/javascript">
	...
	function up(){
		...
		var f= document.controls;
		
		f.action = "<%=cp%>/join/change.do?userId=${userId}&pageNum=${currentPage}";
		f.submit();
	...
	function BLIND(){
		...
		f.action = "<%=cp%>/sboard/handling.do?num=${num}${params }&reportnum=2";
		f.submit();
	...
	function no(){
		...
		var f = document.reportForm;
		f.action = "<%=cp%>/sboard/handling.do?num=${num}${params }&reportnum=3";
		f.submit();
...
<td style="padding:0px; margin:0px; width: 910px;" rowspan="2" colspan="2">
	<form action="" method="post" name="controls">
	<table style=" width:100%;" border="0" align="center">

		<tr height="60px;">
			<td  align="center" colspan="2">
			<font style="font-size: 20px; font-weight: bolder;">회원관리</font></td>
			<td colspan="6" align="right"><input type="text" name="searchValue">
			<input type="button" value="검색" class="btn2" onclick="renewal();"/></td>
		</tr>
		
		<tr style="background-color: #BDBDBD;" height="30px;" >
		<td align="center" style="width: 90px;">아이디
		</td>
		
			<td align="center"  style="width: 100px;">이름</td>
			<td align="center" style="width: 80px;">성별</td>
			<td align="center" style="width: 200px;">이메일</td>
			<td align="center" style="width: 120px;">전화번호</td>
			<td align="center" style="width: 100px;">등급</td>
			<td align="center" style="width: 110px;">포인트</td>
			<td style="width: 100px;" align="center">관리</td>
			
			</tr>
			
			
			<c:forEach var="dto" items="${lists}">
		<tr style="border-bottom : 1px solid; border-color: #B2CCFF; height: 30px">
		
			<td style="padding-left: 15px; ">${dto.userId}</td>
			
			<td align="center">${dto.userName}</td>
			<td align="center" >${dto.userSex}</td>
			<td align="center">${dto.userEmail}</td>
			<td align="center">${dto.userTel}</td>
			<td align="center" >
			<c:if test="${dto.userGrade==1}">
			일반회원</c:if>
			<c:if test="${dto.userGrade==5}">
			관리자
			</c:if>
			</td>
			<td align="right">
			...
			<input type="text" style="margin-left:10px; width: 90px;text-align: right; " value="${dto.userPoint}" name="userPoint">
			</c:if>
			<c:if test="${dto.userId!=userId }">
			${dto.userPoint}
			</c:if>
			...
			<a href="javascript:location.href='<%=cp %>/sboard/control.do?userId=${dto.userId}&searchValue=${searchValue }&pageNum=${currentPage }';">정보수정</a>
			</c:if>
			
			<c:if test="${userId==dto.userId}">
			<a href="javascript:up()">수정</a>/
			<a href="javascript:location.href='<%=cp%>/join/delete.do?userId=${dto.userId}';">삭제</a> </td>
			</c:if>
		</tr>
		
		</c:forEach>
		
		<c:forEach begin="0" end="${max-list}" step="1">
			<tr>
...
<tr>
<td colspan="8" align="center" style="height: 30px;">${pageIndexList }</td>
</tr>
...
<tr style="height: 300px;">
<td colspan="0" style="width: 260px;">
	<table border="0" style="margin: 10px;">	
	<tr height="280px;">
	<td width="240px;" align="char"><img alt="" src="<%=cp%>/images/punch.png" width="230px" height="230px"> </td>
		</tr>
	</table>
...
<td colspan="2" width="950px;" style="margin: 0px;padding: 0px;">
<form name="reportForm" method="post">
	<table border="0" width="100%">
		<tr>
			...
			<font style="font-size: 13px;"color="#ffffff"><span class="glyphicon glyphicon-question-sign" style="margin: 5px 5px 5px 10px;"></span> <b>신고게시물</b></font></td>
			<td align="right">
			
					<select name="reportKey" class="selectFiled">
						<option value="non" selected="selected">선택</option>
						<option value="board">게시판</option>
						<option value="subject">제목</option>
						<option value="userId">글쓴이</option>
						
					</select>
					<input type="text" name="reportValue" class="textFiled"/>
					<input type="button" value="검색" class="btn2" onclick="report();" name="wq"/>
		...
		<tr style="height: 250px;">
			...
			<tr style="background-color: #BDBDBD;" height="32px;">
			<td style="width: 50px;" align="center">No</td>
			<td style="width: 100px;" align="center">게시판</td>
			<td style="widows: 300px;" align="center">제목</td>
			<td align="center" style="width: 100px;">글쓴이</td>
			<td style="width: 100px;" align="center">처리상태</td>
			<td style="width: 150px;" align="center">신고/처리날짜</td>
			<td style="width: 150px;" align="center">수정하기</td>
			</tr>
			
			<c:forEach var="board" items="${report}">
			<tr style="border-bottom : 1px solid; border-color: #B2CCFF; height: 32px">
			<Td align="center">${board.rnum }</Td><td align="center">${board.board}</td>
			<td align="center">
			<a href="javascript:location.href='<%=cp%>/sboard/view.do?board=${board.board}&num=${board.num}';">${board.subject}</a></td>
			<td align="center">${board.userId}</td>
			<td align="center">
			<c:if test="${board.reportCount==1 }">
			신고접수
			</c:if>
			<c:if test="${board.reportCount==2 }">
			블라인드처리
			</c:if>
			<c:if test="${board.reportCount==3 }">
			문제없음
			</c:if>
			</td>
			<td align="center">${board.reportDate}</td>
			
			<td align="center">
			<c:if test="${rnum!=board.rnum }">
			<a href="javascript:location.href='<%=cp %>/sboard/control.do?rnum=${board.rnum}&reportValue=${reportValue }&pageNum1=${reportPage }&num=${board.num}&board=${board.board }';">처리</a>
			</c:if>
			<c:if test="${rnum==board.rnum}">
			<a href="javascript:BLIND()" >블라인드</a>	/<a href="javascript:no()">문제없음</a>
			</c:if>
			</td>
			</tr>
			</c:forEach>
			...
			<c:forEach begin="0" end="${remax-repostsize}" step="1">
			<tr>
			...
			<tr>
			<td colspan="7" align="center">${reportIndexList}</td>
			</tr>
